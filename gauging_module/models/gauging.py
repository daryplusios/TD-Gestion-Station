...
class Gauging(models.Model):
    ...
    # Grille de décompte des billets (Photo 4)
...
class GaugingPumpLine(models.Model):
    ...
    price_unit = fields.Float(string='Prix Unitaire du Gallon', related='product_id.list_price', store=True)
...
class GaugingTankLine(models.Model):
    ...
    opening_qty = fields.Float(string='Niveau à l\'Ouverture')
    closing_qty = fields.Float(string='Niveau à la Fermeture')
...
class GaugingCouponLine(models.Model):
    ...
    unit_value = fields.Float(string='Valeur du Bon')
...
class GaugingDisbursement(models.Model):
    ...
    gauging_id = fields.Many2one('gauging.gauging', ondelete='cascade')
    beneficiary = fields.Char(string='Raison ou Bénéficiaire')
